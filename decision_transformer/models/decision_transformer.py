...
class KnapsackTransformer(TrajectoryModel):
    ...
    def forward(self, states, actions, returns_to_go, timesteps, attention_mask=None):
        ...
        if self.max_length is not None:
            attention_mask = torch.cat(
                [torch.zeros(self.max_length - states.shape[1]), torch.ones(states.shape[1])]).to(states.device)
            attention_mask = attention_mask.reshape(1, -1)
        else:
            attention_mask = torch.ones((1, states.shape[1]), dtype=torch.long).to(states.device)
    ...
    def get_action(self, states, actions, returns_to_go, timesteps, **kwargs):
        ...
        # Apply max_length constraints if needed
        if self.max_length is not None:
        ...
        # Convert probabilities to binary decisions (0: reject, 1: select)
        action_prob = action_preds[0, -1]  # Get the action prediction for the last item
        action_decision = (action_prob > 0.5).float()  # Select if probability > 0.5
